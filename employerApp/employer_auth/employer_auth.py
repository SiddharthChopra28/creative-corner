...
@employer_auth_bp.route('/register', methods=["GET", "POST"], strict_slashes=False)
def register():

    if current_user.is_authenticated:
        return redirect(f'{HTTP_STATUS}{SERVER_NAME}/employer')
    ...
    if request.method == 'POST':
        ...
        if not emailIsUnique is None:
            flash(Markup(
                "An account already exists with this email. <a href=\"{HTTP_STATUS}{SERVER_NAME}/explore/login\" class=\"alert-link\">Login Here.</a>"))
            return render_template('employer_register.html')


        if not emailIsCustomer is None:
            flash(Markup(
                f"This email is already registered as a Customer. <a href=\"{HTTP_STATUS}{SERVER_NAME}/explore/login\" class=\"alert-link\">Login Here.</a> Or try to register with a different email-id."))
...
@employer_auth_bp.route('/register/verify_otp', methods=["GET", "POST"], strict_slashes=False)
def verify_reg_otp():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return redirect(f'{HTTP_STATUS}{SERVER_NAME}/employer/register')
        ...
        if otps[mail][1] != user_otp:
            flash(Markup(
                f"Wrong OTP. Try Again. Or <a href=\"{HTTP_STATUS}{SERVER_NAME}/employer/register\" class=\"alert-link\">Click here</a> to start over.</a>"))
        ...
        if otps[mail][1] == user_otp:
            ...
            try:
                ...
                db.session.commit()
                flash(Markup(
                    f"Account created successfully! <a href=\"{HTTP_STATUS}{SERVER_NAME}/employer/login\" class=\"alert-link\">Login Here.</a>"))
                ...
                db.session.rollback()
                flash(Markup(
                    f"An account already exists with this email. <a href=\"{HTTP_STATUS}{SERVER_NAME}/employer/login\" class=\"alert-link\">Login Here.</a>"))
                return render_template('employer_verifyOtp.html')
...
@employer_auth_bp.route('/login', methods=["GET", "POST"], strict_slashes=False)
def login():
    if current_user.is_authenticated:
        return redirect(f'{HTTP_STATUS}{SERVER_NAME}/employer')
    if request.method == "POST":
        ...
        if user is None:
            flash(Markup(
                f"This Email is not registered yet! <a href=\"{HTTP_STATUS}{SERVER_NAME}/employer/register\" class=\"alert-link\">Signup Here</a>"))
        ...
        if pwd_check:
            ...
            try:
                ...
                return redirect(f'{HTTP_STATUS}{SERVER_NAME}/employer?page=posts')
            except Exception as e:
                print(e)
                flash(
...
@employer_auth_bp.route('/forgotPwd',  methods=["GET", "POST"], strict_slashes=False)
...
@employer_auth_bp.route('/forgotPwd/verify_otp', methods=["GET", "POST"], strict_slashes=False)
def verify_shopkeeper_rec_otp():
    ...
    if request.method == 'POST':
        ...
        if rec_mail not in list(otps.keys()):
            ...
            return redirect(f'{HTTP_STATUS}{SERVER_NAME}/employer/forgotPwd')

        if otps[rec_mail] != user_rec_otp:
            flash(Markup(
                f"Wrong OTP. Try Again. Or <a href=\"{HTTP_STATUS}{SERVER_NAME}/employer/forgotPwd\" class=\"alert-link\">Click here</a> to start over.</a>"))
            return render_template('employer_verifyOtp.html')
...
@employer_auth_bp.route('/logout', strict_slashes=False)
@login_required
def logout():
    logout_user()
    return redirect(f"{HTTP_STATUS}{SERVER_NAME}/employer")
