...
@customer_bp.route('/', strict_slashes=False, methods=['GET', 'POST'])
def home():
    ...
    if request.args.get('delete') == 'True':
        ...
        flash(Markup(f"<a href=\"{HTTP_STATUS}{SERVER_NAME}/explore?delete=True&confirm=True\" class=\"alert-link\">Delete Account Permanently? </a>"))
...
@customer_bp.route('/update_details', methods=["GET", "POST"], strict_slashes=False)
@login_required
    ...
    if request.args.get('delete') == 'True':
        if request.args.get('confirm') == 'True':
            try:
                ...
                flash(Markup("There was some error! Try again later."))
                return render_template('explorer_updateDetails.html')
        flash(Markup(f"<a href=\"{HTTP_STATUS}{SERVER_NAME}/explore/update_details?delete=True&confirm=True\" class=\"alert-link\">Delete Account Permanently? </a>"))
        return render_template('explorer_updateDetails.html')
